...
/**
 * The video frame position in pipeline. See \ref AgoraVideoModulePosition.
 * The default value is AgoraVideoModulePositionPostCapture.
 */
...
/** Device type name. such as Built-in, USB, HDMI, etc.
 */
...
- AgoraFocalLengthWide:wide camera
- AgoraFocalLengthUltraWide:ultra wide camera
- AgoraFocalLengthTelephoto:telephoto camera*/
...
/** The filter effect options in [setFilterEffectOptions]([AgoraRtcEngineKit setFilterEffectOptions:options:]).
 */
 ...
 * Externally specified cube files must strictly comply with Cube LUT Specification. If the externally specified cube file does not conform to the specification, the filter effects will be turned off.
 *
 * The following is an example of the format specification for Cube files. The cube file starts with "LUT_3D_SIZE", which indicates the cube size. In filter effects, the cube size is generally limited to 32.
 ...
 *  ** ... **
 *
 * the SDK also provides a built-in cube named "built_in_whiten.cube" for whitening. Specifying the path to "built_in_whiten_filter" can make the built-in cube work.
 */
...
* - `YES`: Subscribe to encoded video data only.
* - `NO`: (Default) Subscribe to decoded video data.
*/
 ...
 * The string length must be less than 64 bytes. Supported character
 * scopes are:
 * - All lowercase English letters: a to z.
 * - All uppercase English letters: A to Z.
 * - All numeric characters: 0 to 9.
