...
/// 消息修改
/// 
/// - Parameter messageUId: 将被修改的消息id
...
/// 获取成功后强制更新本地消息
/// - Parameter messages: 消息列表 [最多20条,每个消息对象需包含ConversationType,targetId,channelId, messageUid,sentTime]
...
/// - Parameter targetId: 会话ID[非法报错:INVALID_PARAMETER_TARGETID]
/// - Parameter channelId: 频道ID(非超级群时传nil)[非法报错:INVALID_PARAMETER_CHANNELID]
/// - Parameter messageUIDs: 消息UID列表(最多20条)[数量大于20或为0,类型不对:INVALID_PARAMETER_MESSAGELIST]
/// - Parameter successBlock: 成功的回调 [messages:成功的消息列表，mismatch:失败的消息UID列表]
/// - Parameter errorBlock: 失败的回调 [errorCode:错误码]
/// 
/// mismatch不为空, 则代表本地没有对应的 message:
...
- (void)removeConversation:(RCConversationType)conversationType
                  targetId:(NSString *)targetId
                 channelId:(nullable NSString *)channelId
                completion:(nullable void(^)(BOOL result))completion NS_SWIFT_NAME(removeConversation(_:targetId:channelId:completion:));
...
/// 
/// conversationTypeList 包含超级群会话类型时，超级群会话只搜索已存储到本地的消息记录。
/// objectNameList中类型名个数不能超过 300，超过会被截断。
/// 
/// conversationTypeList 传值时：
/// OC 需转成 NSNumber 传入（例如 @[ @(ConversationType_PRIVATE) ]），
/// Swift 需获取到 rawValue 强转成 NSNumber 传入
...
/// 异步获取某个超级群指定 channel 列表，某段时间范围内的消息数量(子线程回调, 如有UI操作, 请切回主线程)
///
/// - Parameter targetId: 会话 ID
...
/// 获取会话未读 @消息数 (子线程回调, 如有UI操作, 请切回主线程)
...
/// - Parameter targetId: 会话 ID
/// - Parameter channelId: 所属会话的业务标识
/// - Parameter sendTime: 消息发送的时间[毫秒,0代表从第一条开始查询]
