...
/// - Since: 5.1.5
- (void)addConnectionStatusChangeDelegate:(id<RCConnectionStatusChangeDelegate>)delegate NS_SWIFT_NAME(addConnectionStatusChangeDelegate(_:));

/// 移除 IMLib 连接状态监听
/// 
/// - Parameter delegate: 代理
/// - Since: 5.1.5
- (void)removeConnectionStatusChangeDelegate:(id<RCConnectionStatusChangeDelegate>)delegate NS_SWIFT_NAME(removeConnectionStatusChangeDelegate(_:));
...
/// - Parameter delegate: 代理
/// - Since: 5.1.5
- (void)addReceiveMessageDelegate:(id<RCIMClientReceiveMessageDelegate>)delegate NS_SWIFT_NAME(addReceiveMessageDelegate(_:));

/// 移除 IMLib 接收消息监听
/// 
/// - Parameter delegate: 代理
/// - Since: 5.1.5
- (void)removeReceiveMessageDelegate:(id<RCIMClientReceiveMessageDelegate>)delegate NS_SWIFT_NAME(removeReceiveMessageDelegate(_:));
...
/// - Parameter option: 可配置的参数
/// - Parameter complete: 获取成功的回调 [messages：获取到的历史消息数组； code : 获取是否成功，0表示成功，非 0 表示失败，此时 messages 数组可能存在断档]
...
/// - Remark: 消息操作
/// - Since: 5.3.0
- (void)getMessageSendTime:(long)messageId
                completion:(nullable void(^)(long long time))completion;
...
/// - Remark: 消息操作
/// - Since: 5.3.0
- (void)getMessage:(long)messageId
        completion:(nullable void(^)(RCMessage *_Nullable message))completion;
...
/// - Parameter startTime: 会话的时间戳（获取这个时间戳之前的会话列表，0表示从最新开始获取，单位：毫秒）
/// - Parameter completion: 异步回调[会话 RCConversation 的列表]
/// 
/// 此方法会从本地数据库中，读取会话列表。
/// 返回的会话列表按照时间从前往后排列，如果有置顶的会话，则置顶的会话会排列在前面。
...
/// - Parameter completion: 异步回调[会话 RCConversation 的列表]
/// 
/// 此方法会从本地数据库中，读取会话列表，支持单聊、群聊、系统会话。
/// 返回的会话列表按照时间从前往后排列，如果有置顶的会话，则置顶的会话会排列在前面。
/// 
/// - Remark: 会话列表
/// - Since: 5.3.2
...
/// 异步获取会话中某段时间范围内的消息数量(子线程回调, 如有UI操作, 请切回主线程)
///
/// - Parameter conversationType    会话类型
/// - Parameter targetId: 会话 ID
...
- (void)removeConversation:(RCConversationType)conversationType
                  targetId:(NSString *)targetId
                completion:(nullable void(^)(BOOL ret))completion NS_SWIFT_NAME(removeConversation(_:targetId:completion:));
...
- (void)addToBlacklist:(NSString *)userId
               success:(nullable void (^)(void))successBlock
                 error:(nullable void (^)(RCErrorCode status))errorBlock NS_SWIFT_NAME(addToBlacklist(_:success:error:));
...
///     // 不需要app层区分本地通知和远程推送，recordLocalNotificationResponseEvent 内部判断只在本地时统计
...
/// 
/// - Parameter tagId: 标签 ID
/// - Parameter timestamp: 会话的时间戳（获取这个时间戳之前的会话列表，0表示从最新开始获取，单位：毫秒）
/// - Parameter count: 获取的数量（当实际取回的会话数量小于 count 值时，表明已取完数据）
/// - Parameter completion: 异步回调[会话 RCConversation 的列表]
/// 
/// 会话标签功能仅支持单群聊会话、系统会话
...
- (void)testProxy:(RCIMProxy *)proxy testHost:(NSString *)testHost successBlock:(nullable void(^)(void))successBlock errorBlock:(nullable void(^)(RCErrorCode code))errorBlock;

/// 配置 AppGroupId 与代理
/// 
/// - Parameter identifier: AppGroupId
/// - Parameter isMainApp: 是否为宿主app
/// 
...
- (void)getMessageReadReceiptV4:(nonnull NSString *)targetId
                   ...
                   successBlock:(nullable void (^)(NSInteger readCount, NSInteger totalCount,NSArray<RCGroupMessageReaderV2 *> *list))successBlock
                     errorBlock:(nullable void (^)(RCErrorCode status))errorBlock;


/// 发送消息的已读回执，会将区间内的消息在服务端全部添加已读状态，本地不进行处理。需要lib 自己调用清除未读等相关接口处理其他业务。
/// - Parameters:
/// - conversationType: 会话类型
/// - targetId: 会话ID
...
/// - Parameter delegate: 代理
/// - Since: 5.6.7
- (void)setConversationReadStatusDelegate:(nullable id<RCConversationChannelSyncConversationReadStatusDelegate>)delegate;
...
/// - Since: 5.8.1
- (void)subscribeEvent:(RCSubscribeEventRequest *)request completion:(void (^)(RCErrorCode status, NSArray<NSString *> * _Nullable failedUserIds))completion;
...
/// - Since: 5.8.1
- (void)unSubscribeEvent:(RCSubscribeEventRequest *)request completion:(void (^)(RCErrorCode status, NSArray<NSString *> * _Nullable failedUserIds))completion;
...
/// - Since: 5.8.1
- (void)querySubscribeEvent:(RCSubscribeEventRequest *)request completion:(void (^)(RCErrorCode status, NSArray<RCSubscribeInfoEvent *> * _Nullable subscribeEvents))completion;
...
///
/// - Since: 5.8.1
- (void)querySubscribeEvent:(RCSubscribeEventRequest *)request pageSize:(NSInteger)pageSize startIndex:(NSInteger)startIndex completion:(nullable void (^)(RCErrorCode status, NSArray<RCSubscribeInfoEvent *> * _Nullable subscribeEvents))completion;
...
/// - Parameter delegate: 订阅事件监听器。
/// - Since: 5.8.1
- (void)removeSubscribeEventDelegate:(id<RCSubscribeEventDelegate>)delegate NS_SWIFT_NAME(removeSubscribeEventDelegate(_:));
...
/// - Since: 5.10.4
- (void)removeDatabaseStatusDelegate:(id<RCDatabaseStatusDelegate>)delegate NS_SWIFT_NAME(removeDatabaseStatusDelegate(_:));
...
- (void)getGroupMembersByRole:(NSString *)groupId
                      ...
                      success:(void (^)(RCPagingQueryResult<RCGroupMemberInfo *> *result))successBlock
                        error:(void (^)(RCErrorCode errorCode))errorBlock NS_SWIFT_NAME(getGroupMembersByRole(_:role:option:success:error:));
...
/// 删除好友
/// - Parameter userIds: 用户ID列表，一次最多解除 100 个用户
/// - Parameter directionType: 好友方向类型
/// - Parameter successBlock: 成功回调
/// - Parameter errorBlock: 失败回调
///
...
/// 检查好友关系
/// - Parameter userIds: 用户ID列表，一次最多查询 100 个用户
...
/// 根据用户 ID 获取好友信息
/// - Parameter userIds: 用户ID列表，一次最多获取 100 个好友信息
/// - Parameter successBlock: 成功回调
/// - Parameter errorBlock: 失败回调
///
/// - Since: 5.12.0
...
/// 添加流式消息请求事件监听。
/// - Since: 5.16.0
- (void)addStreamMessageRequestEventDelegate:(id<RCStreamMessageRequestEventDelegate>)delegate NS_SWIFT_NAME(addStreamMessageRequestEventDelegate(_:));

/// 移除流式消息请求事件监听。
/// - Since: 5.16.0
- (void)removeStreamMessageRequestEventDelegate:(id<RCStreamMessageRequestEventDelegate>)delegate NS_SWIFT_NAME(removeStreamMessageRequestEventDelegate(_:));
