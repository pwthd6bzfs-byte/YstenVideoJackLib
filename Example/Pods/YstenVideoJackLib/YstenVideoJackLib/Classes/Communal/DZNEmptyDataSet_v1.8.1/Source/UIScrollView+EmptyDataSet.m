...
@property (nonatomic, assign) CGFloat verticalSpaceImage_title;
@property (nonatomic, assign) CGFloat verticalSpaceTitle_description;
@property (nonatomic, assign) CGFloat verticalSpaceDescription_button;
...
- (CGFloat)dzn_verticalSpaceImage_title
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(spaceHeight_image_title_ForEmptyDataSet:)]) {
        return [self.emptyDataSetSource spaceHeight_image_title_ForEmptyDataSet:self];
    }
    return 0.0;
}

- (CGFloat)dzn_verticalSpaceTitle_description
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(spaceHeight_title_description_ForEmptyDataSet:)]) {
        return [self.emptyDataSetSource spaceHeight_title_description_ForEmptyDataSet:self];
    }
    return 0.0;
}

- (CGFloat)dzn_verticalSpaceDescription_button
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(spaceHeight_description_button_ForEmptyDataSet:)]) {
        return [self.emptyDataSetSource spaceHeight_description_button_ForEmptyDataSet :self];
    }
    return 0.0;
}
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (customView) {
            ...
            view.verticalSpaceImage_title = [self dzn_verticalSpaceImage_title];
            view.verticalSpaceTitle_description = [self dzn_verticalSpaceTitle_description];
            view.verticalSpaceDescription_button = [self dzn_verticalSpaceDescription_button];
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        CGFloat verticalSpace = kDefaultSpace ;
        ...
        for (int i = 0; i < subviewStrings.count; i++) {
            ...
            if ([string isEqualToString:@"imageView"]) {
                verticalSpace = self.verticalSpaceImage_title ? : 20;
            }
            if ([string isEqualToString:@"titleLabel"]) {
                verticalSpace = self.verticalSpaceTitle_description ? : 5;
            }
           
            if ([string isEqualToString:@"detailLabel"]) {
                verticalSpace = self.verticalSpaceDescription_button ? : 30;
            }
            
            if (i == subviewStrings.count-2 && [[subviewStrings lastObject] isEqualToString:@"button" ]) {
                 verticalSpace = self.verticalSpaceDescription_button ? : 30;
            }
...
//@"[imageView]-(10@750)-[titleLabel]-(10@750)-[detailLabel]"
//@"[imageView]-(10@750)-[titleLabel]-(10@750)-[detailLabel]-(10@750)-[button]"
