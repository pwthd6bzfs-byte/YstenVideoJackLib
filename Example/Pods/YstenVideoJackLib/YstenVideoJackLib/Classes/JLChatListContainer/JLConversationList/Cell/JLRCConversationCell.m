...
//    RCDynamicColor(@"highlight_color", @"0xf5f5f5", @"0x1c1c1eCC");
    ...
    [self.contentView addSubview:self.headerView]; // 废弃
    ...
    [self.contentView addSubview:self.bubbleViewContent];
    [self.contentView addSubview:self.newHeaderView];  // 新的头像

    // 不需要的UI
    self.statusView.conversationNotificationStatusView.hidden = YES;
    
    self.conversationTagView.hidden = YES;
    self.statusView.hidden = YES;
    self.headerView.bubbleView.hidden = YES;
    self.headerView.hidden = YES;

    self.conversationTitle.textColor = [UIColor colorWithHexString:@"#000000"];
    self.conversationTitle.font = [UIFont boldSystemFontOfSize:15];
    self.messageContentLabel.textColor = [UIColor colorWithHexString:@"#666666"];
    self.messageContentLabel.font = [UIFont systemFontOfSize:(CGFloat)13];
    self.messageCreatedTimeLabel.textColor = [UIColor colorWithHexString:@"#000000"];
    self.messageCreatedTimeLabel.font = [UIFont systemFontOfSize:(CGFloat)10];
    
    [self addSubViewConstraints];
        
    
    [self.newHeaderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.contentView);
        make.left.mas_equalTo(self.headerView);
        make.size.mas_equalTo(45.0);
    }];

    
    [self.bubbleViewContent mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.messageContentLabel);
        make.right.mas_equalTo(self.messageCreatedTimeLabel);
...
//    self.backgroundColor = self.model.isTop ? self.topCellBackgroundColor : self.cellBackgroundColor;
    // 设置置顶的颜色
    self.backgroundColor = self.model.isTop ? [UIColor colorWithHexString:@"#D9D9D9" alpha:0.1] : [UIColor clearColor];

    
    if (model.conversationModelType == RC_CONVERSATION_MODEL_TYPE_NORMAL || model.conversationModelType == RC_CONVERSATION_MODEL_TYPE_CUSTOMIZATION) {
...
//    [self.headerView updateBubbleUnreadNumber:(int)model.unreadMessageCount];
    [self.bubbleView setBubbleTipNumber:(int)model.unreadMessageCount];
...
- (RCloudImageView *)newHeaderView {
    if(!_newHeaderView) {
        _newHeaderView = [[RCloudImageView alloc]
                       initWithFrame:CGRectMake(0, 0, RCKitConfigCenter.ui.globalConversationPortraitSize.width,
                                                RCKitConfigCenter.ui.globalConversationPortraitSize.height)];
        [_newHeaderView
         addGestureRecognizer:[[UILongPressGestureRecognizer alloc] initWithTarget:self
        ...
        _newHeaderView.layer.cornerRadius = 45.0/2;
        _newHeaderView.layer.masksToBounds = YES;
    }
    return _newHeaderView;
}
...
- (UIView *)bubbleViewContent {
    if(!_bubbleViewContent) {
        _bubbleViewContent = [[UIView alloc] init];
    }
    return _bubbleViewContent;
}


- (RCMessageBubbleTipView *)bubbleView {
    if(!_bubbleView) {
        _bubbleView = [[RCMessageBubbleTipView alloc] initWithParentView:self.bubbleViewContent alignment:RC_MESSAGE_BUBBLE_TIP_VIEW_ALIGNMENT_CENTER];
        _bubbleView.bubbleTipBackgroundColor =  RCDynamicColor(@"hint_color", @"0xf43530", @"0xf43530");
...
- (void)setIsShowNotificationNumber:(BOOL)isShowNotificationNumber {
    self.bubbleView.isShowNotificationNumber = isShowNotificationNumber;
}
- (BOOL)isShowNotificationNumber {
    return self.bubbleView.isShowNotificationNumber;
}
@end
