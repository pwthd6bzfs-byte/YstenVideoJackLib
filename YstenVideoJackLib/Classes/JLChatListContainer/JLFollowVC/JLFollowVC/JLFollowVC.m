...
@interface JLFollowVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>

    // 消息TableView
@property (nonatomic, strong) UITableView *tableView;
    // 数据源
@property (nonatomic, strong) JLFollowListModel *model;
...
- (void)headRefreshEvent{
    ...
    [JLAPIService getFollowingInfowithWithPage:self.page size:self.size success:^(NSDictionary * _Nonnull result) {
        ...
        self.model = [JLFollowListModel modelWithJSON:result[@"data"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    JLFollowCell *cell = [tableView dequeueReusableCellWithIdentifier:@"JLFollowCell"];
    
    JLFollowModel *model = self.model.records[indexPath.row];
    cell.model = model;
    
    Weakself(ws);
    cell.clickFollowBtnBlock = ^{
        [ws requestFollowData:model.followUserId];
    };
    return  cell;
}
...
-(UITableView *) tableView
{
    if (!_tableView)
        ...
        [view registerClass:[JLFollowCell class] forCellReuseIdentifier:@"JLFollowCell"];
